...
def test_successful_show_anomalies(tmpdir):
    ...
    Returns:
        None
    """
    from components.tfdv import show_anomalies

    # Read in a correct anomalies string
    anomalies_path = Path("tests/tfdv/assets/correct_anomalies.pbtxt")
...
def test_failed_show_anomalies(tmpdir):
    ...
    anomalies_path = Path("tests/tfdv/assets/incorrect_anomalies.pbtxt")
    anomalies = Artifact(uri=str(anomalies_path))
    fail_on_anomalies = True
